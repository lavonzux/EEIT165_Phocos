...
public class EventController {
	...
	public String insert(@ModelAttribute("EventInfo") Event event) {
	...
	public String deleteEvent(@RequestParam("eventID") Integer eventID) {
		eService.deleteByEventId(eventID);
		return "redirect:/events";
	}
	...
	public String editPage(@RequestParam("eventID") Integer eventID, Model model) {
		Event event = eService.getById(eventID);
		model.addAttribute("event", event);
		return "backstage/event/UpdateEvent";
	}
	
	@PutMapping("/event/editData")
	public String editPost(@ModelAttribute(name="EventInfo") Event event) {
		eService.updateEventById(event.getEventID(), event.getMemberID(), event.getEventType(), 
				event.getEventTopic(), event.getEventInfo(),event.getEventDate());
