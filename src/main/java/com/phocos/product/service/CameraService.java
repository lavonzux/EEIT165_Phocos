...
public class CameraService {
	...
	@Transactional
	// 修改
	public Camera updateCameraById(@RequestParam Integer productID, @RequestParam String cameraModel,
			@RequestParam String cameraBrand, @RequestParam Integer cameraPrice, @RequestParam String cameraSensor,
			@RequestParam Integer cameraPx, @RequestParam String cameraRecPx, @RequestParam String cameraMount,
			@RequestParam String cameraIBIS, @RequestParam String cameraEVF, @RequestParam String cameraLCD,
			@RequestParam String cameraFocusSys, @RequestParam String cameraPhotometry,
			@RequestParam Float cameraISOMin, @RequestParam Integer cameraISOMax, @RequestParam String cameraShutter,
			@RequestParam String cameraBurst, @RequestParam String cameraMemCard, @RequestParam String cameraBattery,
			@RequestParam String cameraDims, @RequestParam Integer cameraWeight, @RequestParam byte[] cameraPhoto,
			@RequestParam int cmaeraStocks) throws IOException {
		Optional<Camera> optional = cameraRepo.findById(productID);
		if (optional.isPresent()) {
...
//////////前台//////////
	public Page<Camera> findBycameraPage(Integer pageNumber) {
		Pageable pgb = PageRequest.of(pageNumber - 1, 12, Sort.Direction.DESC, "cameraBrand");

		Page<Camera> page = cameraRepo.findAll(pgb);
		return page;
	}
