...
public class StudioPicService {
	...
	public StudioPic readFirstByStudioID(int studioID) {
		...
		List<StudioPic> sPic = spRepo.findAllByStudioPicID(optional.get().getStudioPicID());
		if (sPic.size()==0) {
	...
	public List<StudioPic> readAllPicturesByStudioPicID(int studioID) {
		
		System.out.println("checking if "+studioID+" exist......");
		Optional<Studio> optional = sRepo.findById(studioID);
		if (optional.isEmpty()) {
			System.out.println("Cannot find the Studio!");
			return null;
